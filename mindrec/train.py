...
def train(local_rank,
    ...
    try:
        ...
        total_steps = args.epochs * len(data_files)  # Approximation based on epochs and data files
        ...
        for ep in range(args.epochs):
            ...
            for cnt, batch in tqdm(enumerate(dataloader)):
                ...
                if global_step % args.log_steps == 0:
                    elapsed_time = time.time() - start_time - test_time
                    steps_per_sec = global_step / elapsed_time
                    remaining_steps = total_steps - global_step
                    eta = remaining_steps / steps_per_sec if steps_per_sec > 0 else float('inf')

                    logging.info(
                        '[{}] cost_time:{} step:{}, train_loss: {:.5f}, acc:{:.5f}, hit:{}, encode_num:{}, lr:{:.8f}, pretrain_lr:{:.8f}, , ETA: {:.2f}s'.format(
                            ...
                            optimizer.param_groups[1]['lr'], optimizer.param_groups[0]['lr'], eta))
                    loss = 0.0
                    accuary = 0.0
