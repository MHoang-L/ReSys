...
from models.fastformer import FastformerLayer
ffconfig = BertConfig.from_json_file('models/fastconfig.json')
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        """
        bz = x.shape[0]
        e = self.att_fc1(x)  # (batch_size, seq_len, hidden_size // 2)
        e = nn.Tanh()(e)
        alpha = self.att_fc2(e)  # (batch_size, seq_len, 1)

        alpha = torch.exp(alpha)
        if attn_mask is not None:
            alpha = alpha * attn_mask.unsqueeze(2) # (batch_size, seq_len, 1)
        alpha = alpha / (torch.sum(alpha, dim=1, keepdim=True) + 1e-8)

        x = torch.bmm(x.permute(0, 2, 1), alpha) # (batch_size, d_h, 1)
        x = torch.reshape(x, (bz, -1))  # (batch_size, d_h)
        return x
...
class TextEncoder(nn.Module):
    def __init__(self, args):
        ...
        # Add Fastformer layer to encoder
        self.fastformer = FastformerLayer(ffconfig)
    ...
    def sent_encode(self, inputs):
        ...
        sent_vec = self.unicoder(text_ids, text_attmask)[0] # (batch_size, seq_len, hidden_size)

        # Apply Fastformer layer to embedded vectors
        sent_vec = self.fastformer(sent_vec, text_attmask)
        ...
        news_vec = self.fc(sent_vec) # (batch_size, news_dim)
        return news_vec
...
class UserEncoder(nn.Module):
    def __init__(self, args, text_encoder=None):
        ...
        # Add Fastformer layer to UserEncoder
        self.fastformer = FastformerLayer(ffconfig)
    ...
    def get_user_log_vec(
        ...
        # Apply Fastformer to improve representation of user history articles
        sent_vecs = self.fastformer(sent_vecs, log_mask)  # Apply Fastformer to embedded vectors
    ...
    def forward(self, user_news_vecs, log_mask,
                user_log_mask=False):
        """
        user_news_vecs: (batch_size, log_length, news_dim)
        log_mask: (batch_size, log_length)
        """
